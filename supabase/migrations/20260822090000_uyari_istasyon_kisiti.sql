-- =============================================================
-- "Seans açılmadı" uyarısı yalnızca üretim istasyonlarına
--
-- Temizlik istasyonunda şu an sisteme giriş yapılmıyor; oradaki personel
-- için uyarı üretmek yanlış alarm demek. Kişi bazlı uretim_seansi_beklenir
-- bayrağı zaten var ama tek başına yeterli değil: yeni bir temizlik
-- personeli eklenip bayrak yanlışlıkla açık bırakılırsa yine uyarı çıkardı.
--
-- Fonksiyonun geri kalanı (B: 4 saatten uzun açık seanslar, C: vardiya
-- sonu kapatılmamış seanslar) 20260805120000 ile BİREBİR aynı — yalnızca
-- A bölümüne istasyon kısıtı eklendi.
-- =============================================================
...
BEGIN
  ...
  IF saat >= TIME '09:00' AND saat < TIME '09:20' THEN
    ...
    WHERE u.is_active
      ...
      -- Yalnızca seans girişi yapılan istasyonlar. Temizlikte şu an sisteme
      -- kayıt girilmiyor; oradaki personeli uyarmak yanlış alarm demek.
      AND u.station::text IN ('Montaj', 'Kesim', 'Paketleme')
      AND NOT EXISTS (
