...
export function KesimRecords({ records }: KesimRecordsProps) {
  const [expanded, setExpanded] = useState(false);
  const [selectedBatch, setSelectedBatch] = useState<CutBatchRow | null>(null);

  const displayCuts = expanded ? records : records.slice(0, 8);
      ...
      {records.length > 8 && (
        ...
        >
          {expanded ? (
            ...
            <>
              <ChevronDown className="w-3 h-3 mr-1" /> {records.length - 8} kesim daha
            </>
          )}
        </Button>
      )}
