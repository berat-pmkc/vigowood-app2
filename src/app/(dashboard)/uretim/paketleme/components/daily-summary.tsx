...
export function DailySummary({ todayTotal, todayCompleted, todayInProgress, todayProducts }: DailySummaryProps) {
      ...
      {todayProducts.length > 0 && (
        <Card className="p-3">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm font-medium text-muted-foreground">Ürün Bazlı</span>
          </div>
          <div className="space-y-1">
            {todayProducts.map((p) => (
              <div key={p.sku} className="flex justify-between items-center text-sm">
                <span className="truncate flex-1 mr-2">{p.urun_adi}</span>
