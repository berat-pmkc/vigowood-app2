...
export function VigowoodDashboard({
  ...
  const filteredTopProducts = useMemo(() => {
    ...
    const nameMap = new Map(availableSkus.map((s) => [s.sku, s.name]));
    const agg = new Map<string, { sku: string; name: string; quantity: number; revenue: number }>();

    for (const [sku, entries] of Object.entries(skuDailyData)) {
      if (sku === "TÜMÜ") continue;
    ...
    return Array.from(agg.values())
      ...
      .slice(0, 10);
  }, [topFilter, customDate, skuDailyData, availableSkus, selectedMonth, topProducts]);
      ...
      <Card>
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            <CardTitle className="text-sm font-medium">
              En Çok Satan SKU&apos;lar — {topFilterLabel}
            </CardTitle>
            ...
            {filteredTopProducts.length > 0 && (
              <tfoot>
                <tr className="border-t-2 border-border bg-muted/50">
                  ...
                  <td className="py-2 pr-4 text-right font-bold text-emerald-700 tabular-nums text-sm">
