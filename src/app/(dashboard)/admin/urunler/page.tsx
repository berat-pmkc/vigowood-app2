...
export default async function UrunlerPage({ searchParams }: PageProps) {
  ...
  // Fetch data in a separate async scope to avoid let query builder issues
  const { data: products, count, error } = await (async () => {
    ...
    return q.order(sortColumn, { ascending: sortOrder }).range(from, to);
  })();
